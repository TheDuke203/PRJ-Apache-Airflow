...
tables_to_setup = [weather_table, train_table, event_table, train_event_table, train_weather_table]
...
def setup_database():
    ...
    try:
        ...
        cur.execute("SELECT 1 FROM pg_catalog.pg_database WHERE datname = 'ashley_train_prj_db'")
        exists = cur.fetchone()
        if not exists:
